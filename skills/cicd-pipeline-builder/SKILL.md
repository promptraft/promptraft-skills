...
- **Intent Detection:** Classify the user request as one of: create / analyze / refactor / audit / report for the *ci/cd pipeline builder* context
- **Entities to Extract:**
...
- **Sub-tasks:**
  1. Classify the CI/CD Pipeline Builder request: new build vs. refactor vs. audit
...
- **Primary Tool:** `GitHub Copilot`
  - **Role:** Generate initial boilerplate code, suggest autocomplete, and produce type-annotated function stubs based on component contract
- **Secondary Tool:** `ESLint`
  - **Role:** Static analysis with AirBnB ruleset; enforce no-`any`, import order, accessibility jsx-a11y rules, and flag CWV anti-patterns
...
- **Data Flow:** GitHub Copilot produces structured output → normalize to internal schema → ESLint validates against quality gates → pass to Output Optimization
...
- **Confidence Score:** Append `[confidence: XX% · skill: WEB-025]` to every output
...
**Output (JSON):**
```
// The skill will return a JSON-formatted result
