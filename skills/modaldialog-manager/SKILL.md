...
> **Skill ID:** `WEB-077` · **Domain:** Web Development · **Type:** Analytical
...
- **Intent Detection:** Classify the user request as one of: create / analyze / refactor / audit / report for the *modal/dialog manager* context
- **Entities to Extract:**
...
- **Strategy:** Reflexion
- **Sub-tasks:**
  1. Classify the Modal/Dialog Manager request: new build vs. refactor vs. audit
...
- **Primary Tool:** `Webpack Analyzer`
  - **Role:** Produce bundle composition report; flag any chunk exceeding 50 KB gzipped and suggest code-split points
- **Secondary Tool:** `Vercel API`
  - **Role:** Deploy preview environment, retrieve deployment URL, and fetch real-user CWV data from Vercel Analytics
...
- **Data Flow:** Webpack Analyzer produces structured output → normalize to internal schema → Vercel API validates against quality gates → pass to Output Optimization
...
- **Confidence Score:** Append `[confidence: XX% · skill: WEB-077]` to every output
...
**Output (CSV):**
```
// The skill will return a CSV-formatted result
