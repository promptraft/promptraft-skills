---
name: DAT-033-model-explainer-shap-lime
...
- **Intent:** Detect user intent related to *model explainer (shap/lime)*
...
- **Sub-tasks:**
  1. Parse and validate the input for *model explainer (shap/lime)* context
  2. `Construct enterprise DAG strategy matching Model Explainer (SHAP/LIME)`
...
- **Primary Tool:** `Pandas` — `Executes the primary heavy-lifting specific to the domain logic`
- **Secondary Tool:** `Scikit-learn` — `Provides validation, fallback, or post-processing security checks`
- **Execution Order:** `Parallel (unless specifically blocked)`
- **Error Handling:** Retry × 2 → fallback tool → graceful error message
- **Data Flow:** `Pandas output → AST & Schema normalizer → Scikit-learn input`
...
skill:
  id: DAT-033
  name: "Model Explainer (SHAP/LIME)"
  domain: Data Science
  version: 1.0.0
  difficulty: Beginner
  type: Agentic
