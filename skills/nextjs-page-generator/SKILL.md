...
> **Skill ID:** `WEB-006` · **Domain:** Web Development · **Type:** Generative
...
- **Intent Detection:** Classify the user request as one of: create / analyze / refactor / audit / report for the *next.js page generator* context
- **Entities to Extract:**
...
- **Sub-tasks:**
  1. Classify the Next.js Page Generator request: new build vs. refactor vs. audit
...
- **Data Flow:** Vercel API produces structured output → normalize to internal schema → Supabase validates against quality gates → pass to Output Optimization
...
- **Confidence Score:** Append `[confidence: XX% · skill: WEB-006]` to every output
...
**Output (Plain Text):**
```
// The skill will return a Plain Text-formatted result
